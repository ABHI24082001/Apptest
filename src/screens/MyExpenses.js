...
const LeaveReportScreen = ({ navigation }) => {
  ...
  const fetchLeaveData = async () => {
    ...
    try {
      ...
      console.error('Expense/Advance Report API error:', error);
  ...
  const renderAppBar = () => (
      ...
      <Appbar.Header style={styles.header} >
        ...
        <Appbar.Content title="My Expenses Report" titleStyle={styles.headerTitle} />
      </Appbar.Header>
  ...
  const renderExpenseCard = ({ item }) => (
    <Card style={styles.card} elevation={2}>
      ...
      <Card.Content>
        ...
        <View style={styles.row}>
          <Chip style={styles.chip}>Total: {item.totalAmount}</Chip>
          <Chip style={styles.chip}>Approved: {item.approvalAmount}</Chip>
        </View>
        <View style={styles.row}>
          <Card style={styles.dateCard} elevation={0}>
            <Card.Content>
              <Text style={styles.dateLabel}>Transaction</Text>
              <Text style={styles.dateValue}>{formatDate(item.transactionDate)}</Text>
            </Card.Content>
          </Card>
          <Card style={styles.dateCard} elevation={0}>
            <Card.Content>
