...
import {InferenceSession, Tensor, ort} from 'onnxruntime-react-native';
// Icons
...
// Config
const INPUT_SIZE = Platform.select({
  android: 112,
  ios: 224,
});
...
const HomeScreen = () => {
  ...
  const SKIP_LOCATION_CHECK = false;
  const SKIP_FACE_CHECK = false;
  ...
  // Platform-specific background task for Android
  const androidBackgroundTask = async taskData => {
    ...
    try {
      while (BackgroundService.isRunning()) {
        try {
          ...
          consecutiveErrors++;
          console.error(
            `Android background task error ${consecutiveErrors}:`,
            innerError,
          );

          if (consecutiveErrors >= maxConsecutiveErrors) {
            console.error(
              'Too many consecutive errors, stopping Android background task',
            );
            break;
          }
      ...
      console.error('Fatal Android background task error:', err);
    }
  };
  ...
  const configureIOSBackgroundFetch = async () => {
    try {
      await BackgroundFetch.configure(
        ...
        async taskId => {
          console.log('🍎 iOS Background fetch started:', taskId);

          try {
            const elapsedData = await AsyncStorage.getItem(BG_LAST_ELAPSED_KEY);
            let elapsedSeconds = elapsedData ? parseInt(elapsedData, 10) : 0;
            elapsedSeconds += 15; // Increment by fetch interval
            ...
            console.log(
              '🍎 iOS Background fetch completed, elapsed:',
              elapsedSeconds,
            );
          } catch (error) {
            console.error('🍎 iOS Background fetch error:', error);
          }
        ...
        error => {
          console.error('🍎 iOS Background fetch configure error:', error);
        },
      );
    } catch (error) {
      console.error('🍎 iOS Background fetch setup failed:', error);
    }
  };
  ...
  const startBackgroundService = async () => {
    try {
      if (Platform.OS === 'android') {
        // Android: Use react-native-background-actions
        const isRunning = BackgroundService.isRunning();
        if (isRunning) {
          console.log(
            '🤖 Android background service already running, stopping first...',
          );
        ...
        androidBackgroundTask(bgOptions.parameters).catch(error => {
          console.error('🤖 Android background task failed to start:', error);
          setTimeout(() => {
            if (BackgroundService.isRunning()) {
              androidBackgroundTask(bgOptions.parameters).catch(console.error);
            }
          }, 5000);
        });
        ...
        // iOS: Use react-native-background-fetch
        await configureIOSBackgroundFetch();

        // Start background fetch
        await BackgroundFetch.start();

        console.log('✅ iOS background fetch started successfully');
      }
    } catch (e) {
      console.error(
        `❌ Failed to start background service on ${Platform.OS}:`,
        e,
      );
    }
  };
  ...
  const stopBackgroundService = async () => {
    try {
      ...
      console.error(
        `❌ Failed to stop background service on ${Platform.OS}:`,
        e,
      );
    }
  };
  ...
  //     // 2️⃣ Camera options (platform-wise tuned)
  //     const cameraOptions = {
  //       mediaType: 'photo',
  //       includeBase64: true,
  //       cameraType: 'front',
  ...
  //       maxWidth: Platform.OS === 'android' ? 600 : 800,
  //       maxHeight: Platform.OS === 'android' ? 600 : 800,

  //       saveToPhotos: Platform.OS === 'ios', // iOS true, Android false
  //     };

  //     // 3️⃣ Launch camera with timeout safety
  ...
  //           // 4️⃣ Image size validation (avoid memory crash)
  //           const base64 = response.assets[0].base64;
  //           const sizeInBytes = base64.length * 0.75; // approx
  ...
  //             Alert.alert(
  //               'Image too large',
  //               'Please capture a smaller image',
  //             );
  ...
  //     });
  //   } catch (error) {
  //     console.error('❌ launchCamera failed:', error.message);
  //     throw error;
  //   }
  // };
  ...
  const launchCamera = async () => {
    ...
    return new Promise((resolve, reject) => {
      ImagePicker.launchCamera(cameraOptions, response => {
        ...
        if (response.didCancel) {
          return reject(new Error('Camera cancelled'));
        }

        if (response.errorCode) {
          return reject(new Error(response.errorMessage || response.errorCode));
        }

        if (!response.assets?.[0]?.base64) {
          return reject(new Error('No image captured'));
        }
  ...
  const handleCheckIn = async () => {
    // Skip face registration check when both verifications are skipped
    if (!SKIP_FACE_CHECK && !registeredFace) {
      Alert.alert(
        'Registration Required',
        'Please register your face first to enable check-in.',
      );
    ...
    try {
      ...
      // ===============================
      // STEP 1: LOCATION CHECK (OPTIONAL)
      // ===============================
      if (!SKIP_LOCATION_CHECK) {
        ...
        console.log('⏭️ Location check skipped');
      }
      ...
      if (!SKIP_FACE_CHECK) {
        ...
        faceVerificationResult = await new Promise((resolve, reject) => {
          Alert.alert(
            'Face Verification',
            'Please capture your face for verification',
            [
      ...
      // ===============================
      // STEP 4: UPDATE STATE & START PROGRESS BAR
      // ===============================
      const checkInMs = Date.now();
      setCheckedIn(true);
      setCheckInTime(checkInMs);

      console.log('✅ Check-in successful, starting background service...');
      ...
      console.log('📊 Starting shift progress...');
      startShiftProgress(0);
    } catch (error) {
      console.error('❌ Check-in error:', error);
  ...
  const preprocessImage = useCallback(async base64Image => {
    try {
      ...
      const targetSize = Platform.OS === 'android' ? 112 : 224;
      ...
      const resized = await ImageResizer.createResizedImage(
        ...
        'JPEG',
        Platform.select({
          android: 85, // Lower quality for Android
          ios: 100, // Higher quality for iOS
        }),
        0,
        null,
        false,
